...
5. **Mapped network drive / SMB / shared drive path issue? OR ticket/work notes mention VMSPFSFS, VMSPFSFSPG, NFS host, or a \\\\server share path that is inaccessible?** -> Shared File Service (Share Drives) Issues. Use PRC `Shared Drive Server / Infrastructure Failure` when the server itself is down or unreachable.
...
**Mandatory for every ticket — including Excluded.** Output exactly these five labeled lines, in this order, each on its own line, with no markdown / asterisks / quotation marks around the values:
...
4. **Detailed Root Cause (≤9 words, FREE TEXT — specific to this ticket):** [Write a short factual phrase (≤9 words) describing what actually caused THIS specific ticket, derived from the work notes. Do NOT use a template label or copy the PRC. Be specific: name the component, action, or condition that caused the failure. Examples: `VMSPFSFSPG002 NFS server unavailable after network change`, `F3 license blocks desktop Office app activation`, `OneDrive client hung after VPN reconnect`, `P-drive path lost after PC rebuild`. If work notes are silent on cause, write `Root cause not documented in work notes`.]
5. **Issue (mandatory — 1 fresh sentence):** [Plainly state what the user reported and its impact, written fresh in your own words from THIS ticket's description/work notes - e.g. "Access code emails from an external sender were not delivered to the user's Outlook mailbox." Do NOT copy the short_description verbatim; write a clear, standalone sentence a reader unfamiliar with the ticket can understand.]
6. **Root Cause Narrative (mandatory — 1–2 fresh sentences):** [Write, in your own words, what the work notes show actually caused the issue for THIS specific ticket. This must be freshly generated by reading the incident's own work notes - never the catalog Possible Root Cause label, never copied from another field. If the work notes do not conclusively prove the cause, say so plainly (e.g. "root cause not confirmed beyond recipient-side filtering behavior") rather than guessing.]

**If using "Excluded" category, also include:**
Exclusion Reason: [Brief description of why the ticket is out of scope, e.g. "Out of scope — cellular service" or "Out of scope — PC hardware replacement". This is in addition to — not a replacement for — Possible Root Cause, Issue, and Root Cause Narrative.]
...
Reasoning: [This becomes the "AI Analysis" shown to the reader alongside the Problem block — it is a CATEGORY-JUSTIFICATION paragraph, not a retelling of the incident. Do NOT restate the Issue, Root Cause Narrative, Resolution Summary, or Key Evidence verbatim — assume the reader already saw them. Instead explain WHY the evidence supports this Primary Category, Sub-symptom, and Confidence Level over the next-most-plausible alternative category: reference the specific application/area identified, the resolution indicators that matched, and any licensing constraints (Copilot blackout, F3 restriction) that drove the decision. Write it the way you would justify a judgment call to a peer reviewer — state explicitly which alternative categories were ruled out and why.
...
- Example of CORRECT output:
    ...
    Possible Root Cause: Copilot License Blackout  
    Issue: The user reported Copilot was not visible in Excel and PowerPoint even though it still worked in Outlook.  
    Root Cause Narrative: The work notes show the agent confirmed in AGS that the "MSOL License – Copilot for M365" entitlement was hidden because the existing license inventory was depleted during a contract blackout period, not because of a client-side or permission fault.  
    Confidence Level: High  
    Reasoning: There is direct AGS evidence of a depleted, hidden entitlement rather than a client bug or permission fault, which is why this fits Microsoft 365 Copilot Issues over Microsoft Excel Issues or Microsoft PowerPoint Issues — the failure follows the license, not the app, since Outlook (no Copilot claim removed) was unaffected. Confidence is High because the entitlement name, blackout cause, and remediation path are all explicitly documented rather than inferred.  
...
- **Three-axis discipline:** Category = the **product** from `TicketCategorisation_ProductivityTools.md`, Sub-symptom = the **symptom** the user saw from `TrendSubCategorisation_ProductivityTools.md`, Possible Root Cause = a **short canonical label** picked verbatim from `PossibleRootCause_ProductivityTools.md` (typically 2–5 words — never a sentence). Ticket-specific narrative belongs in Issue, Root Cause Narrative, and Reasoning only — never in the Possible Root Cause label.
- **`Unknown` is a last resort, not a default.** Every ticket — including Excluded — must have a concrete Possible Root Cause label, a fresh Issue sentence, a fresh Root Cause Narrative, and a Reasoning paragraph that justifies the category/confidence choice. For Excluded tickets, the Root Cause label is the canonical Excluded entry (e.g. `Out-of-scope Service Offering`) and the Reasoning must explain why it is out of scope and where it belongs.
