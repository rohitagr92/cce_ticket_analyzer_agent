...
if ($loadedCount -eq $requiredFiles.Count) {
    Write-Host "✓ Successfully loaded all $loadedCount prompt templates" -ForegroundColor Green
} else {
    Write-Host "⚠ Loaded $loadedCount/$($requiredFiles.Count) prompt templates. Failed: $($failedFiles -join ', ')" -ForegroundColor Yellow
}
